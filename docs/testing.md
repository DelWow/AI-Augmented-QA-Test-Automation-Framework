...
Detailed workflows for the commands in the [project README](../README.md). See [configuration and troubleshooting](configuration.md) for environment setup.
...
The initial baseline set is for macOS ARM64 and records the exact Chrome version. On another platform, create and inspect the appropriate baseline explicitly with `npm run visual:update`. Comparison requests the Chrome version recorded in the manifest; change that version only through the reviewed update workflow. Routine checks never update expected images. See the [visual workflow and review instructions](../qa/visual/README.md) before updating baselines.
